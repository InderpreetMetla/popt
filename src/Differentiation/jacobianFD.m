function Jtotal = jacobianFD(x)
%*********************************************************************%
% This function uses Finite Differences to compute the Jacobian of the
% NLP. It does so in a sparse manner by deriving the optimal control
...
for iphase = 1:nphases
    ...
    PCst = c(tgr,xgr,ugr,auxdata);
    % Compute Boundary Constraints
    BCst = b(t0,tf,x0,xf,auxdata);
    ...
    ex = speye(Nx);
    for iState = 1:Nx
        dFdx{iState} = (a(tgr,xgr+hgr.*ex(iState,:),ugr,auxdata)-...
            F)./hgr(:,iState);
        if Np > 0 && MAIN.dependency(iphase).pathcst.state
            dPdx{iState} = (c(tgr,xgr+hgr.*ex(iState,:),ugr,auxdata)-...
                PCst)./hgr(:,iState);
        else
        ...
        if Nb > 0
            if MAIN.dependency(iphase).bndcst.initialstate
                % Boundary Cst wrt x0
                dBdx0{iState} = (b(t0,tf,x0+hx(1,:).*ex(iState,:),...
                    xf,auxdata)-BCst)./hx(1,iState);
            else
                dBdx0{iState} = sparse(1,Nb);
            end
            if MAIN.dependency(iphase).bndcst.finalstate
                % Boundary Cst wrt x0
                dBdxf{iState} = (b(t0,tf,x0,xf+hx(end,:).*ex(iState,:),...
                    auxdata)-BCst)./hx(end,iState);
            else
    ...
    for iControl = 1:Nu
        ...
        dFdu{iControl} = (a(tgr,xgr,ugr + hu.*eu(iControl,:),auxdata)...
            -F)./hu(:,iControl);
        if Np > 0 && MAIN.dependency(iphase).pathcst.control
            dPdu{iControl} = (c(tgr,xgr,ugr+hu.*eu(iControl,:),auxdata)...
                -PCst)./hu(:,iControl);
        else
    ...
    dFdt = (a(tgr+htgr,xgr,ugr,auxdata)-F)./htgr;
    if Np > 0
        dPdt = (c(tgr+htgr,xgr,ugr,auxdata)-PCst)./htgr;
    end

    if Nb > 0
        % Boundary Cst wrt t0
        if MAIN.dependency(iphase).bndcst.initialtime
            dBdt0 = (b(t0+ht0,tf,x0,xf,auxdata)-BCst)./ht0;
        else
            dBdt0 = sparse(1,Nb);
        end
        % Boundary Cst wrt tf
        if MAIN.dependency(iphase).bndcst.finaltime
            dBdtf = (b(t0,tf+htf,x0,xf,auxdata)-BCst)./htf;
        else
            dBdtf = sparse(1,Nb);
        end
    end
